...
class Orchestrator:
    ...
    def run_task(self, task_path:str|Path)->RunResult:
        ...
        if fact_pack_only:
            ...
            run={'run_id':run_id,'task_id':task['task_id'],'workflow_id':wf.id,'workflow_version':wf.version,'status':'completed','step_status':ctx['step_status'],'rework_history':ctx['rework_history'],'completed_at':datetime.now(timezone.utc).isoformat(),'task_source':str(task_file.relative_to(self.root)) if task_file.is_relative_to(self.root) else str(task_file),'requested_workflow_id':task.get('workflow') or task.get('type'),'effective_workflow_id':wf.id,'requested_analysis_mode':task.get('analysis_mode'),'effective_analysis_mode':wf.id,'fact_pack_only':fact_pack_only,'fact_mode':os.getenv('HOS_FACT_MODE'),'force_refresh':os.getenv('HOS_FACT_FORCE_REFRESH','').lower()=='true','final_quality_status':ctx['data_sufficiency_gate']['status']}
