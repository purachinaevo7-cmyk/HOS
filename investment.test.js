...
localStorage.setItem(ic.IC_KEY,JSON.stringify([{code:'2222',name:'Legacy',marketData:{price:900},decision:{targetPrice:1000,investmentReasons:['legacy'],mainRisk:'old'},scores:{dividend:10},themes:['AI'],investmentPurposes:['配当'],riskFlags:[]} ])); let legacy=ic.loadStocks()[0]; assert.equal(legacy.code,'2222'); assert.equal(legacy.companyEvaluation.rank,'D'); assert.equal(legacy.overallEvaluation.decision,'優先度低'); assert.equal(ic.filterStocks([legacy],{companyRank:'D'}).length,1);
